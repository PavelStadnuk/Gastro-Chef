...
import { getAllPrograms, getProgramRows } from '../../api/nutritionPrograms.api.js';
...
const [currentDayIndex, setCurrentDayIndex] = useState<number>(0);
  ...
  const ourPasses = [
    ...
    { text: 'Ежедневная удобная и бесплатная доставка с 6:00 до 10:00', image: delivery },
    { text: 'Сохраняем Вашу энергию и до 14 часов в неделю освобождая от готовки!', image: energy }
  ];
  const shareMas = [
      ...
      <div className={style.ourPasses}>
        {programs.map((program, index) => (
          <div
            ...
            className={currentProgramIndex === index ? style.active : ''}
          >
            <h2>{program.name}</h2>
      ...
      <div className={style.wrapperPrograms}>
        ...
        <div>
          <div className={style.days}>
            {programs[currentProgramIndex].plan.map((day, index) => (
              <h1
                key={index}
                onClick={() => setCurrentDayIndex(index)}
                className={currentDayIndex === index ? style.active : ''}
              >
                {day.weekDay}
              </h1>
            ))}
          ...
          <div className={style.dayPlans}>
            {programs[currentProgramIndex].plan[currentDayIndex]?.meals?.map((meal, mealIndex) => (
              <div key={mealIndex} className={style.dayPlan}>
                <h3 className={style.mealName}>{meal.meal}</h3>
                <p className={style.mealTime}>{meal.timeMeal}</p>
                <div className={style.productInformation}>
                  <div>
                    {meal.description?.map((item, descIndex) => (
                      <div key={descIndex} className={style.planName}>
                        <p>{item.name}</p>
                      </div>
                    ))}
                  </div>
